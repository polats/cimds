document.addEventListener('DOMContentLoaded', function () {
  ...
  function initGrid() {
    ...
    $.getJSON( "item", function( data){
      ...
      grid = new Muuri(gridElement, {
        items: converttoElements(JSONItems), // generateElements(20),
        layoutDuration: 400,
        layoutEasing: 'ease',
        dragEnabled: true,
        dragSortInterval: 50,
  ...
  function converttoElements(source) {
    var ret = [];
    ...
    for (var i = 0; i < source.length; i++)
    {
      var item = source[i]
      ret.push(
        generateElement(item.id, item.description, 'gray', 3, 3, item.image)
      );
    }
  ...
  function generateElement(id, title, color, width, height, imageURL) {

    var itemElem = document.createElement('div');
    var classNames = 'item h' + height + ' w' + width + ' ' + color;
    var itemTemplate = '' +
        '<a class="' + classNames + '" data-id="' + id + '" data-color="' + color + '" data-title="' + title + '" href="/item/' + id + '">' +
          '<div class="item-content">' +
            '<div class="card">' +
              '<img class=card-portrait src="' + imageURL + '"></img>' +
